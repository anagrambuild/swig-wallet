...
//!
//! This module defines the SolRecurringDestinationLimit action type which enforces
//! recurring limits on SOL token operations to specific destinations within the Swig
//! wallet system. The limit resets after a specified time window.
...
/// Represents a recurring limit on SOL token operations to a specific destination.
///
/// This struct tracks and enforces a maximum amount of SOL that can be
/// sent to a specific destination pubkey within a specified time window. The limit
...
impl SolRecurringDestinationLimit {
    /// Processes a SOL operation to this destination and updates the remaining limit.
    ///
    ...
    pub fn run(&mut self, lamport_diff: u64, current_slot: u64) -> Result<(), ProgramError> {
        // Reset the limit if the time window has expired and the operation would fit in a fresh window
...
impl Transmutable for SolRecurringDestinationLimit {
    /// Size of the SolRecurringDestinationLimit struct in bytes (32 + 8 + 8 + 8 + 8 = 64)
...
impl<'a> Actionable<'a> for SolRecurringDestinationLimit {
    ...
    /// Multiple recurring destination limits can exist per role (for different destinations)
    ...
    fn valid_layout(data: &'a [u8]) -> Result<bool, ProgramError> {
        ...
        let recurring_amount = u64::from_le_bytes(data[32..40].try_into().map_err(|_| SwigStateError::InvalidActionData)?);
        let window = u64::from_le_bytes(data[40..48].try_into().map_err(|_| SwigStateError::InvalidActionData)?);
        let _last_reset = u64::from_le_bytes(data[48..56].try_into().map_err(|_| SwigStateError::InvalidActionData)?);
        let current_amount = u64::from_le_bytes(data[56..64].try_into().map_err(|_| SwigStateError::InvalidActionData)?);
        
        // Validate that the destination is a valid pubkey (32 bytes, all zeros is valid)
