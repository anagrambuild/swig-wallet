...
use crate::{
    error::SwigError,
    instruction::{
        accounts::{Context, SignV1Accounts}, Authenticatable, SwigInstruction
    },
    AccountClassification,
};
use swig_assertions::*;
use swig_state_x::{
    swig::{swig_account_signer, Swig, SwigWithRolesMut}, Discriminator, IntoBytes, Transmutable, TransmutableMut
};
...
pub fn sign_v1(
    ...
    let mut swig = SwigWithRolesMut::from_bytes(swig_account_data)?;
    if let Some(authority) = swig.lookup_role(sign_v1.args.role_id)? {
        let authority = swig.get_authority(&authority)?;
