        ...
        for (index, account) in account_classifiers.iter().enumerate() {
            match account {
                ...
                AccountClassification::SwigStakeAccount { state, balance } => {
                    let data =
                        unsafe { &all_accounts.get_unchecked(index).borrow_data_unchecked() };
                    // Get authorized staker
                    let staker = unsafe { data.get_unchecked(12..44) };
                    // Get authorized withdrawer
                    let withdrawer = unsafe { data.get_unchecked(44..76) };
                    let swig_key = ctx.accounts.swig.key().as_ref();
                    if staker != swig_key || withdrawer != swig_key {
