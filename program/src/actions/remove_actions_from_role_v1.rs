...
use swig_state_x::{
    action::{all::All, manage_authority::ManageAuthority, manage_authorization_lock::ManageAuthorizationLock, Action, Permission},
    role::Position,
...
use crate::{
    error::SwigError,
    instruction::{
        accounts::{RemoveActionsFromRoleV1Accounts, Context},
        SwigInstruction,
    },
};
...
impl<'a> RemoveActionsFromRoleV1<'a> {
    /// Parses the instruction data bytes into a RemoveActionsFromRoleV1 instance.
    ///
    /// # Arguments
...
pub fn remove_actions_from_role_v1(
    ...
    let remove_actions_from_role_v1 = RemoveActionsFromRoleV1::from_instruction_bytes(remove).map_err(|e| {
    ...
    let (target_role_offset, target_role_boundary, total_removal_size) = {
        ...
        let acting_role = Swig::get_mut_role(remove_actions_from_role_v1.args.acting_role_id, swig_roles)?;
        ...
        // Check permissions and extract permission flags
        ...
        let has_all = all.is_some();
        let has_manage_auth_lock = manage_auth_lock.is_some();
        ...
        let target_position = unsafe {
            Position::load_unchecked(&swig_roles[target_role_offset..target_role_offset + Position::LEN])?
        };
        ...
        let actions_start = target_role_offset + Position::LEN + target_position.authority_length() as usize;
        ...
        while action_cursor < actions_data.len() {
            ...
            if indices.contains(&action_index) {
                // Check if this action is AuthorizationLock and verify permission
    ...
    let (actions_start, actions_end, authority_length) = {
        let target_position = unsafe {
            Position::load_unchecked(&swig_builder.role_buffer[target_role_offset..target_role_offset + Position::LEN])?
        };
        let actions_start = target_role_offset + Position::LEN + target_position.authority_length() as usize;
    ...
    while actions_start + action_cursor < actions_end {
        let action = unsafe {
            Action::load_unchecked(&swig_builder.role_buffer[actions_start + action_cursor..actions_start + action_cursor + Action::LEN])?
        };
        ...
        if !indices.contains(&action_index) {
            ...
            let action_data = swig_builder.role_buffer[actions_start + action_cursor..actions_start + action_cursor + action_size].to_vec();
    ...
    let position = unsafe {
        Position::load_mut_unchecked(&mut swig_builder.role_buffer[target_role_offset..target_role_offset + Position::LEN])?
    };
    position.num_actions = kept_actions.len() as u16;
    position.boundary = (target_role_offset + Position::LEN + authority_length + actions_size) as u32;
