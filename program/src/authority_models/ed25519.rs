...
pub fn authenticate(authority_data: &[u8], authority_payload: &[u8], account_infos: &[AccountInfo]) -> Result<(), SwigError> {
    ...
    let auth_account = account_infos.get(authority_payload[0] as usize).ok_or(SwigError::InvalidAuthorityPayload)?;
