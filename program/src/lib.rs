...
/// - Swig accounts (the first one must be at index 0 for signing/permission checking)
        ...
        &crate::ID => {
            ...
            match first_byte {
                ...
                Discriminator::SwigAccount if index != 0 => {
                    // Additional Swig accounts are only allowed if the first account is also a Swig account
