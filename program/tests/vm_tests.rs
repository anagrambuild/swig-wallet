...
fn test_token_account_owner_validation() {
    ...
    // Add the recipient_ata as the last account in the instruction accounts vector
    let mut execute_ix_mut = execute_ix;
    execute_ix_mut
        .accounts
    ...
    let execute_transaction =
        VersionedTransaction::try_new(VersionedMessage::V0(message), &[&wallet]).unwrap();
    ...
    let chunk_value = i64::from_le_bytes(chunk_bytes);
    wrong_comparison_instructions.push(VMInstruction::PushValue { value: chunk_value });
    wrong_comparison_instructions.push(VMInstruction::Equal);
    ...
    let wrong_init_result = context.svm.send_transaction(wrong_init_transaction);
    assert!(
    ...
    // Add the recipient_ata as the last account in the instruction accounts vector
    let mut wrong_execute_ix_mut = wrong_execute_ix;
    ...
    let wrong_execute_transaction =
        VersionedTransaction::try_new(VersionedMessage::V0(wrong_message), &[&wallet]).unwrap();

    let wrong_execute_result = context.svm.send_transaction(wrong_execute_transaction);
    println!("Wrong owner validation result: {:?}", wrong_execute_result);

    // The wrong owner validation should return 0 (false) because the comparison
    // will fail Since we're using the wrong owner pubkey for comparison but the
    // correct ATA
...
fn test_plugin_bytecode_token_validation() {
    ...
    // Use the proper ExecutePluginBytecodeInstruction with Some account indices
    let account_indices = Some(vec![0u8]);
    let execute_plugin_ix = swig_interface::ExecutePluginBytecodeInstruction::new(
        plugin_bytecode_account,
        token_program_id,
        wallet.pubkey(), // Use wallet as a temp placeholder for result account
        wallet.pubkey(),
        account_indices,
    )
    .unwrap();

    // Add the recipient_ata as the remaining account in the instruction accounts
    // vector
    ...
    let execute_plugin_result = context.svm.send_transaction(execute_plugin_tx);
    println!(
