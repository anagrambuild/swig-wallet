...
/// Test that validates only authorities with "All" or "ManageAuthorizationLocks" 
/// permissions can remove authorization locks, while others are denied.
#[test_log::test]
fn test_remove_authorization_lock_permission_enforcement() {
    ...
    println!("Testing that only authorities with proper permissions can remove authorization locks");
    println!();
    ...
    // Add authority with limited token permissions (should NOT be able to manage auth locks)
    let token_action = ClientAction::TokenLimit(TokenLimit {
        token_mint: mint_pubkey.to_bytes(),
        current_amount: 1000,
    });

    let add_token_authority_ix = swig_interface::AddAuthorityInstruction::new_with_ed25519_authority(
        swig,
        context.default_payer.pubkey(),
        swig_authority.pubkey(),
        0, // Acting role ID (swig_authority has All permissions)
    ...
    assert!(add_token_authority_result.is_ok(), "Adding token authority should succeed");
    ...
    // Add first lock using swig_authority (All permission)
    ...
    // Add second lock using manage_auth_locks_authority 
    ...
    // Add third lock using swig_authority
    ...
    // Add all three locks
    for (i, lock_ix) in [add_lock1_ix, add_lock2_ix, add_lock3_ix].iter().enumerate() {
        let authority = if i == 1 { &manage_auth_locks_authority } else { &swig_authority };
    ...
    println!("  1. Remove lock using authority with All permission (role 0): Should PASS");
    println!("  2. Remove lock using authority with ManageAuthorizationLocks permission (role 1): Should PASS");
    println!("  3. Remove lock using authority with TokenLimit permission only (role 2): Should FAIL");
    println!();

    // Test 1: Authority with All permission should succeed in removing lock
    println!("EXECUTING TEST SCENARIO 1:");
    println!("Removing authorization lock at index 1 using authority with All permission (role 0)");
    ...
    let remove_lock_all_ix = swig_interface::RemoveAuthorizationLockInstruction::new(
        ...
        context.default_payer.pubkey(),
        0, // acting_role_id: swig_authority (All permission)
        1, // Remove the middle lock (index 1)
    ).unwrap();
    ...
    assert!(remove_lock_all_result.is_ok(), "Authority with All permission should be able to remove authorization lock");
    println!("✅ Scenario 1 PASSED: Authority with All permission successfully removed authorization lock");

    // Verify the lock was removed and count decreased
    let swig_account = context.svm.get_account(&swig).unwrap();
    let swig_with_roles = SwigWithRoles::from_bytes(&swig_account.data).unwrap();
    assert_eq!(swig_with_roles.state.authorization_locks, 2);
    println!("   → Verified: Authorization lock count = {}", swig_with_roles.state.authorization_locks);

    // Verify the correct lock was removed (the 300 amount lock at index 1)
    let (remaining_locks, remaining_count) = swig_with_roles.get_authorization_locks_for_test::<10>().unwrap();
    println!("   → Remaining locks after removal:");
    for i in 0..remaining_count {
        if let Some(lock) = remaining_locks[i] {
            println!("     Lock {}: amount={}", i, lock.amount);
        }
    }
    // Should have locks with amounts 500 and 200 (the 300 amount lock was removed)
    assert_eq!(remaining_locks[0].unwrap().amount, 500);
    assert_eq!(remaining_locks[1].unwrap().amount, 200);
    println!("   → Verified: Correct lock was removed (middle lock with 300 amount)");
    println!();
    ...
    println!("Removing authorization lock at index 0 using authority with ManageAuthorizationLocks permission (role 1)");
    ...
    let remove_lock_manage_ix = swig_interface::RemoveAuthorizationLockInstruction::new(
        ...
        0, // Remove the first lock (index 0)
    ).unwrap();
    ...
    assert!(remove_lock_manage_result.is_ok(), "Authority with ManageAuthorizationLocks permission should be able to remove authorization lock");
    println!("✅ Scenario 2 PASSED: Authority with ManageAuthorizationLocks permission successfully removed authorization lock");
    ...
    // Should only have the 200 amount lock remaining
    let (final_locks, final_count) = swig_with_roles.get_authorization_locks_for_test::<10>().unwrap();
    assert_eq!(final_count, 1);
    assert_eq!(final_locks[0].unwrap().amount, 200);
    println!("   → Verified: Only the 200 amount lock remains");
    println!();
    ...
    println!("Attempting to remove authorization lock using authority with TokenLimit permission only (role 2)");
    ...
    assert!(remove_lock_token_result.is_err(), "Authority with only TokenLimit permission should NOT be able to remove authorization lock");
    println!("✅ Scenario 3 PASSED: Authority with TokenLimit permission was correctly denied");

    // Verify the lock count didn't change
    let swig_account = context.svm.get_account(&swig).unwrap();
    let swig_with_roles = SwigWithRoles::from_bytes(&swig_account.data).unwrap();
    assert_eq!(swig_with_roles.state.authorization_locks, 1, "Lock count should remain unchanged after failed attempt");
    println!("   → Verified: Authorization lock count remains = {}", swig_with_roles.state.authorization_locks);
    println!();
    
    println!("✅ REMOVE AUTHORIZATION LOCK PERMISSION ENFORCEMENT TEST COMPLETED SUCCESSFULLY!");
    println!("✅ Only authorities with All or ManageAuthorizationLocks permissions can remove authorization locks");
...
/// Test that validates edge cases and error conditions for remove authorization lock
#[test_log::test]
fn test_remove_authorization_lock_edge_cases() {
    ...
    assert!(remove_empty_result.is_err(), "Removing lock when no locks exist should fail");
    ...
    println!("Attempting to remove authorization lock with invalid index (1 when only index 0 exists)");
    ...
    assert!(remove_invalid_result.is_err(), "Removing lock with invalid index should fail");
    ...
    assert!(remove_valid_result.is_ok(), "Removing valid lock should succeed");
    ...
    println!("   → Verified: No authorization locks remain (count = {})", swig_with_roles.state.authorization_locks);
    println!();
    ...
    assert!(add_lock_again_result.is_ok(), "Adding lock for role test should succeed");
    ...
    assert!(remove_bad_role_result.is_err(), "Using non-existent role ID should fail");
