...
fn test_remove_authorization_lock_permission_enforcement() {
    ...
    for (i, lock_ix) in [add_lock1_ix, add_lock2_ix, add_lock3_ix].iter().enumerate() {
        let authority = if i < 2 { &manage_auth_locks_authority } else { &swig_authority };
    ...
    println!("  1. Remove lock using authority with All permission (role 0) - removing another role's lock: Should FAIL");
    println!("  2. Remove lock using authority with ManageAuthorizationLocks permission (role 1) - removing their own lock: Should PASS");
    println!("  3. Remove lock using authority with TokenLimit permission only (role 2): Should FAIL");
    println!();

    // Test 1: Authority with All permission should FAIL when trying to remove another role's lock
    println!("EXECUTING TEST SCENARIO 1:");
    println!("Attempting to remove authorization lock at index 1 using authority with All permission (role 0) - removing another role's lock");
    ...
    assert!(remove_lock_all_result.is_err(), "Authority with All permission should NOT be able to remove another role's authorization lock");
    println!("✅ Scenario 1 PASSED: Authority with All permission correctly denied from removing another role's lock");
    ...
    println!("Removing authorization lock at index 0 using authority with ManageAuthorizationLocks permission (role 1) - removing their own lock");
    ...
    assert!(remove_lock_manage_result.is_ok(), "Authority with ManageAuthorizationLocks permission should be able to remove their own authorization lock");
    println!("✅ Scenario 2 PASSED: Authority with ManageAuthorizationLocks permission successfully removed their own authorization lock");
    ...
    // Should have locks with amounts 300 and 200 remaining (the 500 amount lock was removed)
    ...
    let swig_account = context.svm.get_account(&swig).unwrap();
    let swig_with_roles = SwigWithRoles::from_bytes(&swig_account.data).unwrap();
    assert_eq!(swig_with_roles.state.authorization_locks, 2, "Lock count should remain unchanged after failed attempt");
    println!("   → Verified: Authorization lock count remains = {}", swig_with_roles.state.authorization_locks);
    println!();
    ...
    println!("✅ Roles with ManageAuthorizationLocks permissions can only remove their own authorization locks");
