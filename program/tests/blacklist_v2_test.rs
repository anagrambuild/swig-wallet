...
use swig_interface::{AddAuthorityInstruction, AuthorityConfig, ClientAction, SignV2Instruction};
use swig_state::{
    action::{
...
#[test_log::test]
fn test_blacklist_program_prevents_cpi_v2() {
    ...
    let (swig_wallet_address, _) = Pubkey::find_program_address(
        ...
        &Pubkey::from(swig_interface::swig::ID),
    );

    // Fund the swig wallet address with SOL
    context.svm.airdrop(&swig_wallet_address, amount).unwrap();
    ...
    let transfer_ix =
        system_instruction::transfer(&swig_wallet_address, &recipient.pubkey(), amount / 4);
...
#[test_log::test]
fn test_blacklist_wallet_prevents_transaction_v2() {
    let mut context = setup_test_context().unwrap();
    let swig_authority = Keypair::new();
    ...
    let transfer_ix =
        system_instruction::transfer(&swig_wallet_address, &blacklisted_wallet, amount / 4);
...
#[test_log::test]
fn test_blacklist_with_program_permission_v2() {
    let mut context = setup_test_context().unwrap();
    let swig_authority = Keypair::new();
...
#[test_log::test]
fn test_multiple_blacklist_entries_v2() {
    let mut context = setup_test_context().unwrap();
    let swig_authority = Keypair::new();
...
#[test_log::test]
fn test_add_multiple_blacklists_v2() {
    let mut context = setup_test_context().unwrap();
    let swig_authority = Keypair::new();
    ...
    assert_eq!(role.position.num_actions(), 7);
    ...
    let transfer_ix2 =
        system_instruction::transfer(&swig_wallet_address, &blacklisted_wallet1, amount / 4);
    let sign_ix2 = SignV2Instruction::new_ed25519(
        swig_key,
        swig_wallet_address,
        secondary_authority.pubkey(),
    ...
    let transfer_ix3 =
        system_instruction::transfer(&swig_wallet_address, &blacklisted_wallet2, amount / 4);
    let sign_ix3 = SignV2Instruction::new_ed25519(
        swig_key,
        swig_wallet_address,
        secondary_authority.pubkey(),
        transfer_ix3,
    ...
    let transfer_ix4 =
        system_instruction::transfer(&swig_wallet_address, &non_blacklisted_wallet, amount / 4);
    let sign_ix4 = SignV2Instruction::new_ed25519(
        swig_key,
        swig_wallet_address,
        secondary_authority.pubkey(),
        transfer_ix4,
