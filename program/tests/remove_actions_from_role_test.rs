...
use swig_state_x::{
    action::{
        all::All, manage_authority::ManageAuthority, program::Program, sol_limit::SolLimit, Action,
        Permission,
    },
...
fn build_remove_actions_from_role_ix_data(
    ...
    let args =
        RemoveActionsFromRoleV1Args::new(acting_role_id, target_role_id, indices.len() as u16);
...
fn test_remove_actions_from_role_success() {
    ...
    // Add second authority with multiple actions: SolLimit, Program, another
    // SolLimit
    ...
    assert!(
        result.is_ok(),
        "Failed to remove actions from role: {:?}",
        result
    );
    ...
    assert_eq!(
        all_actions.len(),
        2,
        "Should have exactly 2 actions after removal"
    );
...
fn test_remove_multiple_actions_from_role_success() {
    ...
    assert!(
        result.is_ok(),
        "Failed to remove multiple actions from role: {:?}",
        result
    );
    ...
    println!(
        "After removal: position.num_actions() = {}",
        role.position.num_actions()
    );
    let all_actions = role.get_all_actions().unwrap();
    println!(
        "After removal: get_all_actions().len() = {}",
        all_actions.len()
    );
    ...
    let mut cursor = 0;
    let mut idx = 0;
    while cursor < actions_data.len() && idx < 5 {
        // Limit to 5 to avoid infinite loop
        ...
        println!(
            "Action {}: type={}, len={}, boundary={}, cursor={}",
            idx, action_type, action_len, action_boundary, cursor
        );

        if action_boundary as usize <= cursor || action_boundary as usize > actions_data.len() {
            println!(
                "Invalid boundary - would go to {} but actions_data.len() is {}",
                action_boundary,
                actions_data.len()
            );
            break;
...
fn test_remove_actions_from_role_no_permission() {
    ...
    // Try to remove actions using the second authority which doesn't have
    // permission
...
fn test_remove_actions_from_role_remove_all_actions() {
    ...
    assert!(
        result.is_err(),
        "Should fail when trying to remove all actions from a role"
    );
}
