...
/// permissions can add authorization locks, while others are denied.
#[test_log::test]
fn test_authorization_lock_permission_enforcement() {
    ...
    println!("Testing that only authorities with proper permissions can manage authorization locks");
    println!();
    ...
    assert!(add_lock_all_result.is_ok(), "Authority with All permission should be able to add authorization lock");
    println!("✅ Scenario 1 PASSED: Authority with All permission successfully added authorization lock");
    ...
    println!("Adding authorization lock using authority with ManageAuthorizationLocks permission (role 1)");
    ...
    assert!(add_lock_manage_result.is_ok(), "Authority with ManageAuthorizationLocks permission should be able to add authorization lock");
    println!("✅ Scenario 2 PASSED: Authority with ManageAuthorizationLocks permission successfully added authorization lock");
    ...
    println!("Attempting to add authorization lock using authority with TokenLimit permission only (role 2)");
    ...
    assert!(add_lock_token_result.is_err(), "Authority with only TokenLimit permission should NOT be able to add authorization lock");
    ...
    println!("✅ Only authorities with All or ManageAuthorizationLocks permissions can manage authorization locks");
...
fn test_authorization_lock_invalid_role_handling() {
    ...
    assert!(add_lock_invalid_role_result.is_err(), "Using non-existent role ID should fail");
    ...
    assert!(add_lock_valid_role_result.is_ok(), "Using valid role ID should succeed");
...
fn test_authorization_lock_expiry_validation() {
    ...
    let expired_slot = if current_slot > 0 { current_slot - 1 } else { 0 };
    
    println!("TEST SCENARIO 1:");
    println!("Attempting to add authorization lock with expired slot: {}", expired_slot);
    ...
    println!("TEST SCENARIO 2:");
    println!("Adding authorization lock with future expiry slot: {}", future_slot);
    ...
    assert!(add_valid_lock_result.is_ok(), "Adding valid authorization lock should succeed");
