    ...
    for action in actions {
        match action {
            ...
            ClientAction::ManageAuthority(manage) => {
                let action_header =
                    Action::client_new(Permission::ManageAuthority, ManageAuthority::LEN as u16);
                actions_data.extend_from_slice(action_header.into_bytes().unwrap());
...
fn test_add_actions_to_role_success() {
    ...
    assert_eq!(
        all_actions.len(),
        3,
        "Should have exactly 3 actions, but got {}",
        all_actions.len()
    );
}
...
fn test_add_actions_to_role_no_permission() {
    ...
    let new_actions = vec![ClientAction::Program(Program {
        program_id: [4; 32],
    })];
