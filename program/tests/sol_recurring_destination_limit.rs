...
//!
//! This module contains comprehensive tests for the SolRecurringDestinationLimit action,
//! including basic functionality, time window resets, edge cases, and integration with
//! other limit types.
...
fn test_sol_recurring_destination_limit_basic() {
    ...
    context.svm.airdrop(&swig_authority.pubkey(), 1_000_000_000).unwrap();
    ...
    context.svm.airdrop(&second_authority.pubkey(), 1_000_000_000).unwrap();
    ...
    let _txn = add_authority_with_ed25519_root(
        ...
        vec![ClientAction::SolRecurringDestinationLimit(recurring_destination_limit)],
    ).unwrap();
    ...
    let dest_limit = role.get_action::<SolRecurringDestinationLimit>(&recipient.pubkey().to_bytes()).unwrap().unwrap();
    assert_eq!(dest_limit.current_amount, recurring_amount - transfer_amount);
}
...
fn test_sol_recurring_destination_limit_exceeds_limit() {
    ...
    if let Err(e) = res {
        ...
        assert!(matches!(e.err, TransactionError::InstructionError(_, InstructionError::Custom(3027))));
    }
}
...
fn test_sol_recurring_destination_limit_time_reset() {
    ...
    let role = swig_state.get_role(1).unwrap().unwrap();
    let dest_limit = role.get_action::<SolRecurringDestinationLimit>(&recipient.pubkey().to_bytes()).unwrap().unwrap();
    assert_eq!(dest_limit.current_amount, recurring_amount - transfer_amount1);
    ...
    let dest_limit_final = role_final.get_action::<SolRecurringDestinationLimit>(&recipient.pubkey().to_bytes()).unwrap().unwrap();
    assert_eq!(dest_limit_final.current_amount, recurring_amount - transfer_amount2);
...
fn test_multiple_sol_recurring_destination_limits() {
    ...
    context.svm.airdrop(&recipient1.pubkey(), 1_000_000_000).unwrap();
    context.svm.airdrop(&recipient2.pubkey(), 1_000_000_000).unwrap();
    context.svm.airdrop(&swig_authority.pubkey(), 1_000_000_000).unwrap();
    ...
    let combined_tx = VersionedTransaction::try_new(
        VersionedMessage::V0(combined_message),
        &[&second_authority],
    ).unwrap();
    ...
    let dest_limit1 = role.get_action::<SolRecurringDestinationLimit>(&recipient1.pubkey().to_bytes()).unwrap().unwrap();
    assert_eq!(dest_limit1.current_amount, recurring_amount1 - transfer_amount1);
    
    let dest_limit2 = role.get_action::<SolRecurringDestinationLimit>(&recipient2.pubkey().to_bytes()).unwrap().unwrap();
    assert_eq!(dest_limit2.current_amount, recurring_amount2 - transfer_amount2);
}

/// Test recurring destination limit that doesn't reset because transfer exceeds fresh limit
#[test_log::test]
