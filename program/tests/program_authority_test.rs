...
// Test program ID - matches the declared ID in test-program-authority/src/lib.rs
...
// Test program instruction discriminators (must match test-program-authority/src/processor.rs)
...
fn deploy_test_program(context: &mut SwigTestContext) -> anyhow::Result<()> {
    let program_data = std::fs::read(TEST_PROGRAM_PATH)
        .map_err(|e| anyhow::anyhow!("Failed to read test program: {}. Make sure to run `cargo build-sbf` first.", e))?;
...
fn create_program_exec_authority_data(
    program_id: Pubkey,
    instruction_prefix: &[u8],
) -> Vec<u8> {
    ...
    data[IX_PREFIX_OFFSET..IX_PREFIX_OFFSET + instruction_prefix.len()].copy_from_slice(instruction_prefix);
    data
}
...
fn test_create_program_exec_authority() {
    ...
    assert!(result.is_ok(), "Failed to add ProgramExec authority: {:?}", result.err());

    // Verify the authority was added
    let swig_account = context.svm.get_account(&swig_key).unwrap();
    let swig = SwigWithRoles::from_bytes(&swig_account.data).unwrap();

    assert_eq!(swig.state.roles, 2, "Should have 2 roles (root + program exec)");
...
/// Note: UpdateAuthority only modifies permissions/actions, not authority data itself,
/// so to change the discriminator we need to remove and re-add the authority.
#[test_log::test]
fn test_update_program_exec_authority() {
    ...
    // To "update" the discriminator, we need to remove the old authority and add a new one
    // because UpdateAuthority only modifies permissions/actions, not authority data itself
    ...
    assert!(result.is_ok(), "Failed to remove ProgramExec authority: {:?}", result.err());
    ...
    let updated_program_exec_data = create_program_exec_authority_data(
        TEST_PROGRAM_ID,
        &new_discriminator,
    );
    ...
    let program_exec_auth: &ProgramExecAuthority =
        role.authority.as_any().downcast_ref().unwrap();
...
/// This now uses SignV2Instruction::new_program_exec() which returns both the preceding
/// instruction and the sign instruction that must be executed together.
///
...
fn test_program_exec_successful_execution() {
    ...
    let swig_wallet = Pubkey::find_program_address(&swig_wallet_address_seeds(swig.as_ref()), &program_id()).0;
    ...
    // Use the ergonomic interface to create both the preceding and sign instructions
    ...
    assert!(res.is_ok(), "Transaction should succeed with valid program execution and state=0");
}
...
fn test_program_exec_fails_with_state_set_to_fail() {
    ...
    // Create a dummy inner instruction - swig wallet will sign this transfer as a PDA
    // Transfer FROM swig wallet TO authority
    let inner_ix = system_instruction::transfer(&swig_wallet, &swig_authority.pubkey(), 1000);
...
fn test_program_exec_wrong_program_fails() {
    ...
    // Should fail because the preceding instruction is from system program, not TEST_PROGRAM_ID
...
fn test_program_exec_invalid_discriminator_fails() {
    ...
    let swig_wallet_ata = setup_ata(&mut context.svm, &mint_pubkey, &swig_wallet, &context.default_payer).unwrap();
    let recipient_ata = setup_ata(&mut context.svm, &mint_pubkey, &recipient.pubkey(), &recipient).unwrap();

    mint_to(&mut context.svm, &mint_pubkey, &context.default_payer, &swig_wallet_ata, 1000).unwrap();
    ...
    assert!(res.is_err(), "Transaction should fail with invalid discriminator");
...
fn test_program_exec_mismatched_config_fails() {
    ...
    assert!(res.is_err(), "Transaction should fail with mismatched config account");
...
fn test_program_exec_mismatched_wallet_fails() {
    ...
    assert!(res.is_err(), "Transaction should fail with mismatched wallet account");
