...
use swig::actions::{
    ...
    manage_auth_lock_v1::{ManageAuthorizationLocksOperation, ManageAuthorizationLocksV1Args},
...
impl SignV2Instruction {
    ...
    pub fn new_ed25519_with_signers(
        ...
        if let Some(enterprise_account) = enterprise_account {
            accounts.push(AccountMeta::new_readonly(enterprise_account, false));
        }
...
pub enum ManageAuthorizationLocksData {
    AddLock(Vec<ClientAction>),
    RemoveLock(Vec<[u8; 32]>),
    UpdateLock(Vec<ClientAction>),
}

impl ManageAuthorizationLocksData {
    fn to_operation_and_data(self) -> anyhow::Result<(ManageAuthorizationLocksOperation, Vec<u8>)> {
        match self {
            ManageAuthorizationLocksData::AddLock(actions) => Ok((
                ManageAuthorizationLocksOperation::AddLock,
                Self::serialize_actions(actions)?,
            )),
            ManageAuthorizationLocksData::RemoveLock(mints) => Ok((
                ManageAuthorizationLocksOperation::RemoveLock,
                mints.into_iter().flat_map(|mint| mint.to_vec()).collect(),
            )),
            ManageAuthorizationLocksData::UpdateLock(actions) => Ok((
...
pub struct ManageAuthorizationLocksV1Instruction;
impl ManageAuthorizationLocksV1Instruction {
    /// Update authority using Ed25519 signature.
    pub fn new_with_ed25519_authority(
        swig_account: Pubkey,
        payer: Pubkey,
        ...
        acting_role_id: u32,
        authority_to_update_id: u32,
        update_data: ManageAuthorizationLocksData,
    ) -> anyhow::Result<Instruction> {
        let (operation, operation_data) = update_data.to_operation_and_data()?;
        Self::build_ed25519_instruction(
            swig_account,
    ...
    fn build_ed25519_instruction(
        ...
        acting_role_id: u32,
        authority_to_update_id: u32,
        operation: ManageAuthorizationLocksOperation,
        operation_data: Vec<u8>,
    ) -> anyhow::Result<Instruction> {
        let accounts = vec![
            AccountMeta::new(swig_account, false),
